...
MezzConfig GUI delegates released-issue, pull-request, and Discord build
notifications to the shared
...
version uses an independent notifier channel. The project records both the
published tag and the last submitted release commit in its build description,
so it can prevent duplicate publishing without losing the next notification
range. The first submission falls back to Jenkins' previous successful commit.
