...
assert jenkins.getReleaseVersion() == 'mc26.1.2-0.3.0'
assert jenkins.getReleaseGradleArgs() == "-PRELEASE_VERSION='mc26.1.2-0.3.0'"
...
env.RELEASE_TAG = 'mc26.1.2-0.3.0'
env.BRANCH_NAME = 'feature/config-screen'
assert !jenkins.shouldPublishRelease()
env.BRANCH_NAME = '26.1.2'
env.CHANGE_ID = '123'
assert !jenkins.shouldPublishRelease()
env.CHANGE_ID = null
...
currentBuild.previousBuild.previousBuild = [
    result: 'SUCCESS',
    description: jenkins.releaseMarker('mc26.1.2-0.3.0'),
    previousBuild: null
]
assert !jenkins.shouldPublishRelease()
...
def completedDescription = jenkins.completedReleaseDescription('mc26.1.2-0.3.0', previousReleaseCommit)
assert jenkins.buildDescriptionMarkers(completedDescription) == [
    jenkins.releaseMarker('mc26.1.2-0.3.0'),
...
assert jenkins.wasReleasePublished('mc26.1.2-0.3.0'): "currentBuild=${currentBuild.inspect()}"
assert !jenkins.shouldPublishRelease()
assert jenkins.findPreviousReleaseCommit() == previousReleaseCommit
env.RELEASE_TAG = 'mc26.1.2-0.3.1'
assert jenkins.shouldPublishRelease()
env.RELEASE_TAG = 'mc26.1.2-0.3.0'
...
env.RELEASE_TAG = 'refs/tags/mc26.1.2-0.3.0'
assert jenkins.getReleaseVersion() == 'mc26.1.2-0.3.0'
