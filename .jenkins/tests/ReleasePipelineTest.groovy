...
assert jenkins.getReleaseVersion() == 'mc1.21.11-0.3.0'
assert jenkins.getReleaseGradleArgs() == "-PRELEASE_VERSION='mc1.21.11-0.3.0'"
...
env.RELEASE_TAG = 'mc1.21.11-0.3.0'
...
currentBuild.previousBuild.previousBuild = [
    result: 'SUCCESS',
    description: jenkins.releaseMarker('mc1.21.11-0.3.0'),
    previousBuild: null
]
assert !jenkins.shouldPublishRelease()
...
def completedDescription = jenkins.completedReleaseDescription('mc1.21.11-0.3.0', previousReleaseCommit)
assert jenkins.buildDescriptionMarkers(completedDescription) == [
    jenkins.releaseMarker('mc1.21.11-0.3.0'),
...
assert jenkins.wasReleasePublished('mc1.21.11-0.3.0'): "currentBuild=${currentBuild.inspect()}"
assert !jenkins.shouldPublishRelease()
assert jenkins.findPreviousReleaseCommit() == previousReleaseCommit
env.RELEASE_TAG = 'mc1.21.11-0.3.1'
assert jenkins.shouldPublishRelease()
env.RELEASE_TAG = 'mc1.21.11-0.3.0'
...
env.RELEASE_TAG = 'refs/tags/mc1.21.11-0.3.0'
assert jenkins.getReleaseVersion() == 'mc1.21.11-0.3.0'
