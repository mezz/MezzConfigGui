...
versions, and Git tags use `mc<minecraft>-<mod-version>`, for example `mc1.21.1-0.5.3`.
...
3. Tag the verified commit `mc<minecraft>-<mod-version>`. This branch uses
   `mc1.21.1-0.5.3`. Push the intended tag.
...
To validate a release locally, pass `-PRELEASE_VERSION=mc1.21.1-0.5.3` to Gradle.
Untagged development builds append the build number, such as `mc1.21.1-0.5.3.9999`.
