...
versions, and Git tags use `mc<minecraft>-<mod-version>`, for example `mc1.20.1-0.5.2`.
...
3. Tag the verified commit `mc<minecraft>-<mod-version>`. This branch uses
   `mc1.20.1-0.5.2`. Push the intended tag.
...
To validate a release locally, pass `-PRELEASE_VERSION=mc1.20.1-0.5.2` to Gradle.
Untagged development builds append the build number, such as `mc1.20.1-0.5.2.9999`.
