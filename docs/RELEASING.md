...
versions, and Git tags use `mc<minecraft>-<mod-version>`, for example `mc26.2-0.5.1`.
...
3. Tag the verified commit `mc<minecraft>-<mod-version>`. This branch uses
   `mc26.2-0.5.1`. Push the intended tag.
...
To validate a release locally, pass `-PRELEASE_VERSION=mc26.2-0.5.1` to Gradle.
Untagged development builds append the build number, such as `mc26.2-0.5.1.9999`.
