...
versions, and Git tags use `mc<minecraft>-<mod-version>`, for example `mc26.3-0.5.3`.
...
3. Tag the verified commit `mc<minecraft>-<mod-version>`. This branch uses
   `mc26.3-0.5.3`. Push the intended tag.
...
To validate a release locally, pass `-PRELEASE_VERSION=mc26.3-0.5.3` to Gradle.
Untagged development builds append the build number, such as `mc26.3-0.5.3.9999`.
