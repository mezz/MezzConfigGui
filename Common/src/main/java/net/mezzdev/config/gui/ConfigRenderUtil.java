...
public final class ConfigRenderUtil {
	...
	public static void renderEditBox(EditBox box, GuiGraphicsExtractor graphics, int x, int y, float tick) { box.extractRenderState(graphics, x, y, tick); }
	...
	public static void tooltip(GuiGraphicsExtractor graphics, Font font, List<FormattedCharSequence> lines, ClientTooltipPositioner positioner, int x, int y) {
		graphics.tooltip(font, lines.stream().map(ClientTooltipComponent::create).toList(), x, y, positioner, null, false);
	}
