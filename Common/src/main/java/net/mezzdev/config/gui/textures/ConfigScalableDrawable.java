...
public final class ConfigScalableDrawable {
	...
	public void draw(LegacyGuiGraphics graphics, ImmutableRect2i area) { draw(graphics, area.getX(), area.getY(), area.getWidth(), area.getHeight()); }
	public void draw(LegacyGuiGraphics graphics, int x, int y, int width, int height) {
		if (width <= 0 || height <= 0)
			return;
	...
	private static void tile(LegacyGuiGraphics graphics, TextureAtlasSprite sprite, int textureWidth, int textureHeight, int u, int v, int tileWidth, int tileHeight, int x, int y, int width, int height) {
