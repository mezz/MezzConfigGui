...
final class PendingChangesScreen extends MezzConfigScreen {
	...
	@Override
	public void renderBackground(LegacyGuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
		guiGraphics.fill(0, 0, width, height, ConfigGuiColors.getColor(ConfigGuiColors.GuiColor.PENDING_CHANGES_BACKGROUND));
	}

	@Nullable
	private PendingConfigChange drawContent(LegacyGuiGraphics guiGraphics, int mouseX, int mouseY) {
		ScreenLayout screenLayout = getScreenLayout();
	...
	@Nullable
	private PendingConfigChange drawChangeList(LegacyGuiGraphics guiGraphics, ImmutableRect2i area, int mouseX, int mouseY) {
		guiGraphics.fill(
			area.getX(),
			area.getY(),
	...
	private void drawScrollbar(LegacyGuiGraphics guiGraphics, ImmutableRect2i innerArea) {
		if (!hasScrollbar()) {
			return;
		}
	...
	private static void drawTooltip(LegacyGuiGraphics guiGraphics, int mouseX, int mouseY, @Nullable PendingConfigChange hoveredChange) {
	...
	private static void drawBorder(LegacyGuiGraphics guiGraphics, ImmutableRect2i area, int color) {
